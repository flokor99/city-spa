...
function parsePostInput(event) {
  const headers = event.headers || {};
  const q = event.queryStringParameters || {};
  const contentType = String(headers["content-type"] || headers["Content-Type"] || "").toLowerCase();
  const bodyText = decodeRequestBody(event);

  // Fallback-kompatibel: JSON bleibt weiterhin erlaubt.
  if (contentType.includes("application/json")) {
    const body = bodyText ? JSON.parse(bodyText) : {};
    return {
      ...body,
      documentPath: body.documentPath || q.documentPath || headers["x-document-path"],
      ownerUserId: body.ownerUserId || q.ownerUserId || headers["x-owner-user-id"],
      ownerEmail: body.ownerEmail || q.ownerEmail || headers["x-owner-email"],
      documentTitle: body.documentTitle || q.documentTitle || headers["x-document-title"],
      title: body.title || q.title || headers["x-annex-title"],
      description: body.description || q.description || headers["x-annex-description"],
      city: body.city || q.city || headers["x-city"],
    };
  }

  // Neuer Standard: Markdown direkt im Body, Meta über Query/Header.
  return {
    markdown: bodyText,
    documentPath: q.documentPath || headers["x-document-path"] || null,
    ownerUserId: q.ownerUserId || headers["x-owner-user-id"] || null,
    ownerEmail: q.ownerEmail || headers["x-owner-email"] || null,
    documentTitle: q.documentTitle || headers["x-document-title"] || null,
    title: q.title || headers["x-annex-title"] || null,
    description: q.description || headers["x-annex-description"] || null,
    city: q.city || headers["x-city"] || null,
    sources: null,
    background: null,
  };
}
