export const toolLimits = [
  "Keine Rechts-, Steuer- oder Investitionsberatung.",
  "Kann falsche oder unvollständige Aussagen erzeugen.",
  "Aktualität ist nicht garantiert.",
  "Ergebnisse ersetzen keine fachliche Prüfung.",
];

export const presentations = [
  {
    id: "intro",
    title: "City Profiler – Überblick",
    description: "Was das Tool kann. Wofür es gedacht ist.",
    fileUrl: "/docs/presentations/city-profiler_intro.pdf",
    previewUrl: "/assets/preview_intro.png",
  },
];

export const faqItems = [
  {
    q: "Ist das Tool eine verlässliche Entscheidungsgrundlage?",
    a: "Nein. Es ist ein Analyse- und Denkwerkzeug. Ergebnisse müssen eingeordnet und geprüft werden.",
  },
  {
    q: "Kann ich Zahlen und Zitate direkt übernehmen?",
    a: "Nein. Kritische Fakten immer prüfen und Quellen gegenlesen.",
  },
];

export const annexItems = [
  {
    id: "methodik",
    title: "Anhang – Methodikdetails",
    description: "Definitionen, Scoring-Logik, Annahmen.",
    fileUrl: "/docs/annex/methodik.pdf",
  },
  {
    id: "glossar",
    title: "Anhang – Glossar",
    description: "Begriffe und Abkürzungen.",
    fileUrl: "/docs/annex/glossar.pdf",
  },
];
